...
const FALLBACK_BODY = [
  { _type: 'block', _key: '1', style: 'normal', children: [{ _type: 'span', text: 'Като потребител имате право да се откажете от поръчката в рамките на 14 дни от получаването й, без да посочвате причина.' }] },
]

export default async function PravoNaOtkazPage() {
  let page: any = null
  try {
    if (process.env.NEXT_PUBLIC_SANITY_PROJECT_ID) {
      page = await client.fetch(legalPageQuery, { slug: 'pravo-na-otkaz' })
    }
  } catch (_) {}

  const title = page?.title ?? 'Право на отказ и връщане'
